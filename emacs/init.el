...
(leaf *builtin
  ...
  :bind (("C-x C-1" . async-shell-command)
         ("s-n" . make-frame)
         ("s-w" . delete-frame)
         ("s-@" . other-frame)
         ([?¥] . [?\\])
...
(leaf *interface
  ...
  (leaf *appearance
    ...
    (leaf fontaine
      :ensure t
      :hook ((emacs-startup-hook . fontaine-restore-latest-preset)
  ...
  (leaf *nav
    ...
    (leaf block-nav
      :ensure t
      :bind (("C-c C-n" . block-nav-next-block)
    ...
    (defun tempel-include (elt)
      (when (eq (car-safe elt) 'i)
        (if-let (template (alist-get (cadr elt) (tempel--templates)))
          ...
          nil)))
    :ensure t
    :hook ((text-mode-hook . tempel-setup-capf)
           (prog-mode-hook . tempel-setup-capf)
           (conf-omde-hook . tempel-setup-capf))
...
(leaf *programming
  ...
  (leaf lsp-mode
    ...
    :custom ((lsp-ocaml-lsp-server-command . '("ocamllsp" "--fallback-read-dot-merlin"))
  ...
  (leaf claudemacs
    :el-get (claudemacs :type github :pkgname "cpoile/claudemacs")
    :bind (:prog-mode-map
           ("C-c C-c C-l" . claudemacs-transient-menu))
    :config
    (add-to-list 'display-buffer-alist
                 '("^\\*claudemacs"
  ...
  (leaf *latex
    ...
    (leaf pdf-tools
      ...
      :bind (:pdf-view-mode-map
             ("C-s" . isearch-forward)
             ("+" . pdf-view-enlarge))
      :custom ((pdf-view-resize-factor . 1.2))))
...
(leaf *org
  ...
  (leaf org-ref
    ...
    :custom ((bibtex-completion-bibliography . '("~/pro/memo/common/my.bib"))
